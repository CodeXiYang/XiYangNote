...
> 视频: https://www.bilibili.com/video/BV1ZT4y1K75K
>
> - Docker核心概念，是什么、能干什么 Docker整体架构，和传统虚拟机的区别。
>- 深刻理解镜像、容器、仓库的各种概念和操作。
> - 熟练掌握Dockerfile的编写和构建并使用Dockerfile来制作复杂镜像，能够使用容器卷完成容器间数据共享和持久化工作。
> - docker-compose安装使用,以及容器编排工作等相关内容
...
最新官网首页: *docker支持苹果最新发布的M1芯片*

![image-20201220213306128](assets/image-20201220213306128.png)

![image-20201220214210994](assets/image-20201220214210994.png)

```markdown
# 官方介绍
- We have a complete container solution for you 
	# 翻: 我们为你提供了一个完整的容器解决方案
- no matter who you are and where you are on your containerization journey.
	# 翻: 不管你是谁,不管你在哪,你都可以开始容器的的旅程。

# 官方定义
- docker是一个应用容器技术。
- 可以通过docker容器安装各种程序的应用环境
- docker容器保证软件环境版本的一致
```
...
基于 Linux 内核的 cgroup，namespace，以及 OverlayFS 类的 Union FS 等技术，
...
*使用Docker的好处是什么呢?Docker的技术为什么这么火呢?*
...
![image-20201229114531793](assets/image-20201229114531793.png)

- 鲸鱼代表docker引擎,需要使用docker就需要安装docker引擎
...
- 虚拟机是利用Hypervisor去虚拟化内存，整个调用过程是<u>虚拟内存->虚拟物理内存->真正物理内存</u>
- 但是Docker是利用Docker Engine去调用宿主的的资源，这时候过程是<u>虚拟内存->真正物理内存</u>。

*各方面虚拟机与Docker容器的对比如下:*
...
*注意: Docker安装在centos发型版本时候不能低于7版本,centos6无法使用docker*
...
### 12.4 Dockerfile构建springboot项目部署
...
![image-20201223231707738](assets/image-20201223231707738.png)
